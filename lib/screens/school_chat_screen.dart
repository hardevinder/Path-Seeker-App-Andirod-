...
class _SchoolChatScreenState extends State<SchoolChatScreen> {
  ...
  Future<void> _boot() async {
      ...
      onPresence: (p) {
        ...
        setState(() {
          threads = threads.map((t) {
            ...
            return same
                ? <String, dynamic>{...t, 'online': p['online'] == true}
                : t;
          }).toList();
        });
      },
    ...
    if (!_deepLinkOpened && widget.openThreadId != null && mounted) {
      final matches = threads
          .where((t) => '${t['id']}' == '${widget.openThreadId}')
          .toList();
      if (matches.isNotEmpty) {
        _deepLinkOpened = true;
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) _open(matches.first);
        });
      }
    }
  }
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return ListTile(
                        ...
                        title: Text(name),
                        subtitle: Text(
                            '${r['group'] ?? ''}${username.isNotEmpty ? ' • $username' : ''}'),
  ...
  void _open(Map<String, dynamic> thread) {
    Navigator.push(
      ...
      MaterialPageRoute(
          builder: (_) =>
              SchoolChatThreadScreen(service: service, thread: thread)),
    ).then((_) => _load());
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              onPressed: _newChat, icon: const Icon(Icons.add_comment_rounded)),
                          ...
                          title: Text(name,
                              style: TextStyle(
                                  fontWeight: unread > 0
                                      ...
                                      : FontWeight.w700)),
                          subtitle: Text(
                              '${t['lastMessage']?['body'] ?? 'Start conversation'}',
                              maxLines: 1,
...
class SchoolChatThreadScreen extends StatefulWidget {
  ...
  const SchoolChatThreadScreen(
      {super.key, required this.service, required this.thread});
...
class _SchoolChatThreadScreenState extends State<SchoolChatThreadScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (r.isNotEmpty && r.last['own'] != true && last != null)
        widget.service.seen(threadId, last);
      _bottom();
    } catch (_) {
  ...
  void _onMessage(dynamic d) {
    ...
    if (m['own'] != true && m['id'] != null)
      widget.service.seen(threadId, (m['id'] as num).toInt());
    _bottom();
  }
  ...
  void _onTyping(dynamic d) {
    ...
    if (mounted)
      setState(() => typingText =
          d['isTyping'] == true ? '${d['name'] ?? 'Someone'} is typing…' : '');
  }
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (mounted && !messages.any((x) => '${x['id']}' == '${m['id']}'))
  ...
  Future<void> _sendThumbsUp() async {
    ...
    setState(() => sending = true);
    try {
      final m = await widget.service.send(threadId, '👍');
      if (mounted && !messages.any((x) => '${x['id']}' == '${m['id']}')) {
        setState(() => messages.add(m));
      }
      _bottom();
  ...
  Future<void> _attach() async {
    final files = await openFiles(acceptedTypeGroups: [
      const XTypeGroup(label: 'School chat files', extensions: [
        'jpg',
        'jpeg',
        'png',
        'webp',
    ...
    try {
      await widget.service
          .sendFiles(threadId, files, body: controller.text.trim());
      controller.clear();
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context)
  ...
  String _deliveryLabel(Map<String, dynamic> message) {
    final status = '${message['deliveryStatus'] ?? 'sent'}'.toLowerCase();
    if (status != 'seen') {
      return status == 'delivered' ? 'Delivered' : 'Sent';
    }
    final receipts = (message['receipts'] as List? ?? const []);
    DateTime? latestSeen;
    for (final receipt in receipts.whereType<Map>()) {
      final value = receipt['seenAt'] ?? receipt['seen_at'];
      final parsed =
          value == null ? null : DateTime.tryParse('$value')?.toLocal();
      if (parsed != null &&
          (latestSeen == null || parsed.isAfter(latestSeen))) {
        latestSeen = parsed;
      }
    }
    return latestSeen == null
        ? 'Seen'
        : 'Seen at ${DateFormat('dd MMM, hh:mm a').format(latestSeen)}';
  }

  @override
  Widget build(BuildContext context) {
    final other =
        Map<String, dynamic>.from(widget.thread['otherParticipant'] ?? {});
    return Scaffold(
      appBar: AppBar(
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('${other['name'] ?? 'Chat'}'),
          Text(
              widget.thread['online'] == true ? 'Online' : 'Secure school chat',
              style:
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return Align(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                              color: own ? Colors.indigo.shade50 : Colors.white,
                              ...
                              children: [
                                ...
                                for (final a in attachments)
                                  TextButton.icon(
                                    ...
                                    onPressed: () async {
                                      try {
                                        await widget.service.openAttachment(a);
                                      } catch (e) {
                                        if (mounted)
                                          ScaffoldMessenger.of(context)
                                ...
                                Align(
                                    ...
                                    child: Text(
                                        '${_time(m['createdAt'])}${own ? ' • ${_deliveryLabel(m)}' : ''}',
                                        style: const TextStyle(
                                            fontSize: 10, color: Colors.grey))),
                              ]),
                        ),
          ...
          SafeArea(
            ...
            child: Padding(
              ...
              child: Row(children: [
                ...
                IconButton(
                  tooltip: 'Send thumbs up',
                  onPressed: sending ? null : _sendThumbsUp,
