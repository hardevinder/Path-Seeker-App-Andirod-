// lib/screens/teacher/teacher_circulars_screen.dart
import 'dart:async';
...
class _TeacherCircularsScreenState extends State<TeacherCircularsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      // ApiService.fetchCirculars already returns List<Circular>
      final list = await ApiService.fetchCirculars();

      // Keep only teacher/both circulars (case-insensitive)
      _circulars = list.where((c) {
        final aud = (c.audience ?? '').toLowerCase();
        return aud == 'teacher' || aud == 'both' || aud == 'all' || aud == 'everyone';
      }).toList();

      // If none found but API returned items, fall back to showing all
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Couldn't load circulars — try again later.")));
      }
    } finally {
  ...
  List<Circular> get _processed {
    ...
    final sinceDate = _sinceDays == 'all' ? null : DateTime.now().subtract(Duration(days: int.tryParse(_sinceDays) ?? 30));
  ...
  String _formatDT(DateTime dt) => DateFormat.yMMMd().add_jm().format(dt);
  ...
  Future<void> _openUrl(String url) async {
    try {
      ...
      if (await canLaunchUrl(uri)) {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not open attachment')));
      }
    } catch (_) {
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(c.description ?? 'No description', maxLines: 3, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.grey[700])),
              const SizedBox(height: 8),
              Row(children: [
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(color: const Color(0xFFECF5FF), borderRadius: BorderRadius.circular(999)),
                  child: Text(c.audience == 'both' ? 'All Teachers' : 'Teachers', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF1B6ED6))),
                ),
                const SizedBox(width: 8),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Teacher Circulars'),
        elevation: 0,
          ...
          Container(
            ...
            child: Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Teacher Circulars', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white)),
                const SizedBox(height: 6),
                Text('Latest notices & attachments', style: TextStyle(color: Colors.white.withOpacity(0.95))),
              ])),
              const SizedBox(width: 8),
              Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.notifications, color: Colors.white)),
            ]),
          ),

          // Filters
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), child: Column(children: [
            Row(children: [
              Expanded(child: TextField(controller: _searchController, onChanged: _onSearchChanged, decoration: InputDecoration(hintText: 'Search title or description…', filled: true, fillColor: Colors.white, prefixIcon: const Icon(Icons.search), contentPadding: const EdgeInsets.symmetric(vertical: 12), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none)))),
              const SizedBox(width: 8),
              GestureDetector(onTap: () => setState(() => _onlyWithFiles = !_onlyWithFiles), child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10), decoration: BoxDecoration(color: _onlyWithFiles ? const Color(0xFFE7F0FF) : Colors.white, borderRadius: BorderRadius.circular(12)), child: Icon(Icons.attach_file, color: _onlyWithFiles ? const Color(0xFF1F7AE0) : Colors.black54))),
            ]),
          ...
          Expanded(
            child: RefreshIndicator(
              ...
              child: _loading
                  ? _loadingList()
                  : processed.isEmpty
                      ? ListView(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 24), children: const [
                          Center(child: Column(children: [Icon(Icons.inbox, size: 56, color: Colors.black26), SizedBox(height: 12), Text('No circulars found', style: TextStyle(fontWeight: FontWeight.bold)), SizedBox(height: 6), Text('Try changing filters or check back later.', style: TextStyle(color: Colors.black54))]))
                        ])
                      : ListView.separated(
                          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                          itemBuilder: (ctx, i) {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: selected ? const Color(0xFF1F7AE0) : Colors.white, borderRadius: BorderRadius.circular(999), border: Border.all(color: const Color(0xFFE6EEF9))),
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 8),
                Row(children: [
                  Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFECF5FF), borderRadius: BorderRadius.circular(999)), child: Text(circular.audience == 'both' ? 'All Teachers' : 'Teachers', style: const TextStyle(color: Color(0xFF1B6ED6), fontWeight: FontWeight.w700))),
                ...
                if (hasFile) ...[
                  ...
                  if (kind == 'image')
                    ...
                    Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), color: const Color(0xFFF8FAFC)), child: const Text('Preview not available. Use Open / Download.')),
                ],
